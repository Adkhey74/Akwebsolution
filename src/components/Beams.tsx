...
function extendMaterial(
  ...
  cfg: {
    ...
    material?: ConstructorParameters<typeof THREE.MeshStandardMaterial>[0] & { fog?: boolean };
  ...
  if (defaults.color) (uniforms as Record<string, { value: unknown }>).diffuse.value = defaults.color;
  if ("roughness" in defaults) (uniforms as Record<string, { value: unknown }>).roughness.value = defaults.roughness;
  if ("metalness" in defaults) (uniforms as Record<string, { value: unknown }>).metalness.value = defaults.metalness;

  for (const [key, u] of Object.entries(cfg.uniforms ?? {})) {
    (uniforms as Record<string, unknown>)[key] =
...
function createStackedPlanesGeometry(n: number, width: number, height: number, heightSegments: number) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      uvs.set([uvXOff, j / heightSegments + uvYOff, uvXOff + 1, j / heightSegments + uvYOff], ui);
      if (j < heightSegments) {
...
type MergedRef = THREE.Mesh | null;

const MergedPlanes = forwardRef<MergedRef, {
  ...
  useImperativeHandle<THREE.Mesh | null>(ref, () => mesh.current);
...
const PlaneNoise = forwardRef<MergedRef, {
  material: THREE.ShaderMaterial;
  width: number;
  count: number;
  height: number;
...
export function Beams({
  ...
  const meshRef = useRef<MergedRef>(null);
...
vec3 getNormal(vec3 pos){vec3 cur=getCurrentPos(pos);vec3 nx=getCurrentPos(pos+vec3(0.01,0.,0.));vec3 nz=getCurrentPos(pos+vec3(0.,-0.01,0.));return normalize(cross(normalize(nz-cur),normalize(nx-cur)));}`,
        vertex: {
        ...
        material: { fog: true },
        uniforms: {
          diffuse: { value: new THREE.Color(...hexToRGB("#000000")) },
          time: { value: 0 },
          roughness: { value: 0.3 },
          metalness: { value: 0.3 },
          uSpeed: { value: speed },

          ...
          {offers.map((offer, i) => {
            ...
            return (
              ...
              >
                ...
                <div className={`mb-5 flex h-10 w-10 items-center justify-center rounded-xl ${isPro ? "bg-white/10" : "bg-[var(--card)]"}`}>
                  <Icon size={18} strokeWidth={1.75} className={isPro ? "text-white" : "text-[var(--foreground)]"} />
                </div>

                <h3 className={`text-xl font-semibold ${isPro ? "text-white" : "text-[var(--foreground)]"}`}>
                  {offer.title}
                </h3>
                <div className="mt-3">
                  <span className={`text-3xl font-bold tracking-tight ${isPro ? "text-white" : "text-[var(--foreground)]"}`}>
                    {offer.price} €
                  </span>
                  {isPro && (
                    <p className="mt-1 text-[0.75rem] font-medium text-white/60">Sur devis selon votre projet</p>
                  )}
                </div>
                <p className={`mt-2 text-[0.8125rem] leading-relaxed ${isPro ? "text-white/60" : "text-[var(--muted)]"}`}>
                  {offer.target}
                </p>

                <div className={`mt-4 flex items-center gap-2 rounded-lg px-3 py-2 text-[0.8125rem] font-medium ${isPro ? "bg-white/10 text-white/80" : "bg-[var(--card)] text-[var(--muted)]"}`}>
                  <Clock size={12} strokeWidth={2} />
                  Livraison en {offer.delivery}
                </div>

                <div className={`my-5 h-px w-full ${isPro ? "bg-white/10" : "bg-[var(--border)]"}`} />

                <ul className="flex flex-1 flex-col gap-2.5">
                  {offer.features.map((feat) => (
                    <li key={feat} className="flex items-start gap-2.5">
                      <span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-full ${isPro ? "bg-white/15" : "bg-[var(--card)]"}`}>
                        <Check size={9} strokeWidth={2.5} className={isPro ? "text-white" : "text-[var(--foreground)]"} />
                      </span>
                      <span className={`text-[0.8125rem] leading-snug ${isPro ? "text-white/75" : "text-[var(--muted)]"}`}>
                        {feat}
                      </span>
                    </li>
                  ))}
